...
void main() {
  ...
  test(
    'remote reporting is disabled when privacy eligibility is denied',
    () async {
      var emitted = false;
      final reporter = PrivacyGatedCrashReporting(
        configEnabled: true,
  ...
  test(
    'enabled reporting emits only redacted bounded correlation payload',
    () async {
      ...
      await reporter.capture(
        report(
          message: longMessage,
          stackTrace:
              'C:\\Users\\example\\project\\main.dart:10\ntoken=$secret',
        ),
      );
