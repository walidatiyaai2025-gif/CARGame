...
void main() {
  ...
  testWidgets('gameplay ignores cargo reselection while feedback is resolving', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(800, 1000));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    await tester.tap(
      find.byKey(ValueKey('cargo-${selected.id}-0')),
    );
    await tester.pump();
    ...
    final viewport = Offset.zero & tester.binding.renderViews.single.size;
    final selectedTarget = tester.getRect(selectedWarehouse).intersect(viewport);
