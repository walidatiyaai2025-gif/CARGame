...
FILES = {
    ...
    "txqa": ROOT / "Assets/_Project/QA/Editor/SCR_CargoV2TransactionCrashRegression.cs",
    "lifeqa": ROOT / "Assets/_Project/QA/Editor/SCR_CargoV2ActiveDeliveryLifecycleRegression.cs",
...
require("build", "SCR_CargoV2TransactionCrashRegression.ValidateOrThrow();", "transaction crash regression must gate Unity validation/build")
require("build", "SCR_CargoV2ActiveDeliveryLifecycleRegression.ValidateOrThrow();", "active delivery recreation regression must gate Unity validation/build")

for test_name in (
    "TestFreshInstall", "TestProgressHostileSchemas", "TestEconomyIdempotencyAndCorruption",
    "TestCompanySalvageAndFutureSchema", "TestCommittedTransactionCatalogDrift", "TestUnpaidTransactionCatalogDrift",
    "TestActiveDeliveryValidationAndQuarantine", "TestCompletionCrashCleanupAndInvalidHandoffIsolation", "TestFiniteEconomyAndContractMath",
):
    require("qa", test_name, f"missing deterministic regression {test_name}")
for test_name in (
    "TestUnpaidPurchaseCompletesExactlyOnce", "TestCommittedPurchaseFinishesWithoutSecondDebit",
    "TestCompanyCommittedPurchaseOnlyCleansJournal", "TestCommittedUpgradeFinishesWithoutSecondDebit",
    "TestCompanyCommittedUpgradeOnlyCleansJournal", "TestInsufficientUpgradeClearsUnpaidJournalWithoutMutation",
):
    require("txqa", test_name, f"missing transaction crash regression {test_name}")
for cue in ("pre-pickup", "post-pickup", "checkpoint-1", "checkpoint-2", "checkpoint-3"):
    require("lifeqa", cue, f"missing process-recreation state {cue}")
