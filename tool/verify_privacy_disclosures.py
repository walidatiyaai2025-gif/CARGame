...
def _strings(value: object, label: str) -> list[str]:
    ...
    result: list[str] = []
    for item in value:
        _require(isinstance(item, str) and bool(item.strip()), f"{label} must contain non-empty strings")
        result.append(item)
    return result


def _flow_map(flows: list[dict[str, object]], label: str) -> dict[str, dict[str, object]]:
    result: dict[str, dict[str, object]] = {}
    for flow in flows:
        flow_id = flow.get("id") if label == "inventory" else flow.get("flowId")
        _require(isinstance(flow_id, str) and bool(flow_id.strip()), f"{label} flow id is required")
...
def validate_models(
    ...
    _require(disclosure.get("schemaVersion") == 1, "disclosure schemaVersion must be 1")
    _require(
    ...
    inventory_flows = _flow_map(_objects(inventory.get("dataFlows"), "inventory.dataFlows"), "inventory")
    mappings = _flow_map(_objects(disclosure.get("flowMappings"), "disclosure.flowMappings"), "disclosure")
    missing = set(inventory_flows) - set(mappings)
    stale = set(mappings) - set(inventory_flows)
    _require(not missing, "disclosure is missing inventory flows: " + ", ".join(sorted(missing)))
    _require(not stale, "disclosure contains stale flows: " + ", ".join(sorted(stale)))
    ...
    for flow_id, inventory_flow in inventory_flows.items():
        ...
        _require(isinstance(network, bool), f"{flow_id} inventory network flag must be boolean")
        ...
        _require(isinstance(collected, bool), f"{flow_id}.collectedOffDevice must be boolean")
        _require(isinstance(shared, bool), f"{flow_id}.sharedWithThirdParties must be boolean")

        if not network:
            _require(not collected, f"{flow_id} is local-only and must not be declared off-device collection")
            _require(not shared, f"{flow_id} is local-only and must not be declared third-party sharing")
            _require(not data_types, f"{flow_id} is local-only and must not declare Play data types")
            continue

        network_flows.append(flow_id)
        _require(collected, f"{flow_id} network processing must be declared as collected off device")
        _require(shared, f"{flow_id} Google Mobile Ads processing must conservatively be declared shared")
    ...
    typed_ad_data = _objects(ad_mapping.get("dataTypes"), "ad-sdk-processing.dataTypes")
    observed_types: set[str] = set()
    for item in typed_ad_data:
        data_type = item.get("type")
        _require(isinstance(data_type, str) and bool(data_type), "Google Mobile Ads data type id is required")
        _require(data_type not in observed_types, f"duplicate Google Mobile Ads data type: {data_type}")
    ...
    expected_absent = set(_strings(inventory.get("explicitlyAbsent"), "inventory.explicitlyAbsent"))
    disclosed_absent = set(_strings(disclosure.get("explicitlyAbsent"), "disclosure.explicitlyAbsent"))
    _require(
    ...
    _require(safety.get("collectsUserData") is True, "Play Data Safety must disclose SDK off-device collection")
    _require(safety.get("sharesUserData") is True, "Play Data Safety must disclose SDK third-party sharing")
    _require(safety.get("accountCreationAvailable") is False, "CARGame currently has no account creation")
    _require(
        safety.get("deletionRequestMechanismAvailable") is False,
        "deletion request mechanism must remain false until PRIV-003 implements one",
    )
    ...
    deletion_gap = gaps.get("in-app-data-controls")
    _require(
        isinstance(deletion_gap, dict) and deletion_gap.get("owner") == "PRIV-003",
        "PRIV-003 in-app data controls gap must remain explicit while deletion mechanism is unavailable",
    )
    ...
    _require(state in {"draft", "published"}, "publication.state must be draft or published")
    ...
    for token in required_policy_tokens:
        _require(token in policy_text, f"privacy policy is missing required content: {token}")

    if state == "draft":
        _require("Publication status: DRAFT — NOT YET PUBLISHED" in policy_text, "draft policy marker is missing")
        _require(url is None, "draft disclosure must not fabricate a privacy policy URL")
        _require(contact is None, "draft disclosure must not fabricate publisher contact")
        _require(publication.get("targetAudienceConfirmed") is False, "draft target audience must remain unconfirmed")
        _require(publication.get("playConsoleSubmitted") is False, "draft must not claim Play Console submission")
        _require(
        ...
        _require(publication.get("targetAudienceConfirmed") is True, "published target audience must be confirmed")
        _require(publication.get("playConsoleSubmitted") is True, "published state requires Play Console submission")
        _require(
        ...
        _require("NOT YET PUBLISHED" not in policy_text, "published policy still carries a draft marker")
        _require(
...
def validate_repository(root: Path = ROOT) -> tuple[int, int]:
    ...
    try:
        ...
        raise DisclosureError(f"privacy disclosure inputs cannot be read: {exc}") from exc

    validate_models(inventory, disclosure, policy_text)
    mapping_count = len(_objects(disclosure.get("flowMappings"), "disclosure.flowMappings"))
    ad_types = next(
        mapping["dataTypes"]
...
def main() -> int:
    ...
    print(
        ...
        f"{ad_type_count} Google Mobile Ads data types; publication=draft."
    )
    return 0
